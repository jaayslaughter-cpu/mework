...
public class ApifyScraperService {
    ...
    // ── Generic scrape() — called directly by DataHubTasklet ─────────────────

    /**
     * Generic URL scraper used by {@code DataHubTasklet} group loops.
     *
     * <p>DataHubTasklet iterates over {@code PHYSICS_URLS}, {@code CONTEXT_URLS},
     * {@code MARKET_URLS}, and {@code DFS_URLS} and calls {@code scrape(url)} on
     * each.  This method routes to the appropriate typed scraper if one exists,
     * or falls back to the generic table extractor for unknown URLs.</p>
     *
     * @param url Target URL to scrape
     * @return Map of scraped data keyed by field name; empty map on error
     */
    @SuppressWarnings("unchecked")
    public Map<String, Object> scrape(String url) {
        try {
            String json = triggerApifyActor(url);
            if (json == null || json.isBlank() || "[]".equals(json.strip())) {
                log.warn("[Apify] Empty result for URL: {}", url);
                return new HashMap<>();
            }
            // Try to deserialize as a list of row objects first, then as a single object
            ...
            log.error("[Apify] scrape() failed for {}: {}", url, e.getMessage());
            return new HashMap<>();
        }
    }
