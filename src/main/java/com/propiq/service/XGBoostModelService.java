...
public class XGBoostModelService {
    ...
    /**
     * Sends settled bet history to the Python microservice for a SHAP-based
     * feature importance audit.  Called daily by {@code BacktestTasklet}.
     *
     * <p>The Python {@code /api/ml/backtest-audit} endpoint:
     * <ol>
     *   <li>Builds a feature matrix from numeric columns in {@code settledBets}.</li>
     *   <li>Computes overall binary accuracy (xgboost_prob vs prop_hit_actual).</li>
     *   <li>Runs SHAP TreeExplainer to get per-feature mean absolute importances.</li>
     *   <li>Derives per-feature accuracy estimates and flags those below
     *       {@code minFeatureAccuracy}.</li>
     * </ol>
     *
     * @param settledBets       Rows from {@code bet_ledger} as key-value maps.
     * @param minFeatureAccuracy Hard accuracy floor (e.g. 0.777).
     * @return Map with keys: dropped_features (List), feature_accuracies (Map),
     *         overall_accuracy (double), sample_size (int).
     */
    @SuppressWarnings("unchecked")
    public Map<String, Object> runFeatureAudit(
            List<Map<String, Object>> settledBets,
            double minFeatureAccuracy) {

        try {
            Map<String, Object> requestBody = new HashMap<>();
            requestBody.put("settled_bets", settledBets);
            requestBody.put("min_feature_accuracy", minFeatureAccuracy);

            Map<String, Object> response = restTemplate.postForObject(
                    mlServiceUrl + "/api/ml/backtest-audit",
                    requestBody,
                    Map.class
            );

            if (response == null) {
                log.warn("[XGBoostModelService] backtest-audit returned null response.");
                return Map.of(
                        "dropped_features",    List.of(),
                        "feature_accuracies",  Map.of(),
                        "overall_accuracy",    0.0,
                        "sample_size",         settledBets.size()
                );
            }
            ...
            log.error("[XGBoostModelService] runFeatureAudit failed: {}", e.getMessage(), e);
            return Map.of(
                    "dropped_features",    List.of(),
                    "feature_accuracies",  Map.of(),
                    "overall_accuracy",    0.0,
