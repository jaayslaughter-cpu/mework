...
AGENT_SPECS: Dict[str, dict] = {
    ...
    # True ensemble meta-model: VultureStack × 0.60 + UmpireAgent × 0.25 +
    # FadeAgent × 0.15.  Triple-confirmation required — all three constituent
    # agents must agree.  Props must sit at the intersection of all three:
    #   • VultureStack:  runs, earned_runs, hits_runs_rbis  Under
    #   • UmpireAgent:   strikeouts, runs, earned_runs       any side
    #   • FadeAgent:     any Under prop                      Under
    #   Intersection →   runs, earned_runs                  Under ← tightest pool
    #
    # Per-agent additive edges used when computing stacked prob:
    #   VultureStack +0.040 · UmpireAgent +0.020 · FadeAgent +0.015
    # Weighted stacked prob must clear 0.65.
    # Raw min_prob set to 0.62 (proxy for stacked ≥ 0.65 with edge boosts).
            ...
            for prop in BATTER_PROPS:
                ...
                for direction, mp, tp in (
                    ...
                    for agent, spec in AGENT_SPECS.items():
                        ...
                        # OmegaStack: apply weighted ensemble stacking
                        # stacked = 0.60*(mp+0.040) + 0.25*(mp+0.020) + 0.15*(mp+0.015)
            ...
            for prop in PITCHER_PROPS:
                ...
                for direction, mp, tp in (
                    ...
                    for agent, spec in AGENT_SPECS.items():
                        boost     = spec.get("prob_boost", 0.0)
                        mp_agent  = min(mp + boost, 0.99)
                        # OmegaStack: weighted ensemble stacking (pitcher loop)
                        if spec.get("omega_stack"):
                            mp_agent = min(
                                0.60 * (mp + 0.040)
                                + 0.25 * (mp + 0.020)
