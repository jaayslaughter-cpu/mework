...
async def job_settle():
    ...
    FIX: nightly_recap.py lives at repo root, one level above api/.
    The old path joined __file__ (api/orchestrator.py) with "nightly_recap.py"
    → resolved to api/nightly_recap.py which does not exist → subprocess
    exited with code 2 (FileNotFoundError) silently every night.
    """
    _api_dir = os.path.dirname(os.path.abspath(__file__))
    script   = os.path.normpath(os.path.join(_api_dir, "..", "nightly_recap.py"))
    if not os.path.exists(script):
        logger.error(
            "[NightlyRecap] Script not found at %s — settlement skipped. "
            "Ensure nightly_recap.py is at repo root.", script
        )
        return
    # Use await instead of create_task so scheduler captures failures
    await _run_subprocess("NightlyRecap", script)
...
# SECURITY: Restrict CORS to known origins. Set FRONTEND_URL env var in Railway
# (e.g. https://mework.up.railway.app). Comma-separate multiple origins.
