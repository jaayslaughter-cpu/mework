...
def _pitcher_cache_key(mlbam_id: int, season: int) -> str:
    """Weekly Redis/Postgres key for a single pitcher's raw pitch data."""
    today = datetime.now(timezone.utc)
    iso   = today.isocalendar()
    return f"pp_pitches_{mlbam_id}_{season}_{iso[0]}w{iso[1]}"
...
def _fetch_pitcher_pitches(mlbam_id: int, season: int) -> list[dict]:
    ...
    Cache hierarchy (prevents Savant rate-limiting from zeroing out scores):
      L1 — Redis   : key pp_pitches_{mlbam_id}_{season}_{year}w{week}
                     TTL = 7 days (weekly refresh matches scorer cadence)
      L2 — Postgres: layer_cache table via layer_cache_helper
                     Survives Railway redeploys; same weekly key
      L3 — Live Savant CSV fetch with 1.5s request delay
    """
    cache_key = _pitcher_cache_key(mlbam_id, season)

    # ── L1: Redis ────────────────────────────────────────────────────────────
    try:
        import redis as _redis_lib  # noqa: PLC0415
        _r_url = os.environ.get("REDIS_URL") or os.environ.get("REDIS_PRIVATE_URL", "")
        if _r_url:
            _rc = _redis_lib.from_url(_r_url, decode_responses=True)
            cached = _rc.get(cache_key)
            if cached:
                import json as _json  # noqa: PLC0415
                pitches = _json.loads(cached)
                logger.debug(
                    "[PredictPlus] Redis cache hit — mlbam=%d season=%d (%d pitches)",
                    mlbam_id, season, len(pitches),
                )
                return pitches
    except Exception:
        pass  # Redis unavailable — fall through

    # ── L2: Postgres via layer_cache_helper ──────────────────────────────────
    try:
        from layer_cache_helper import pg_cache_get  # noqa: PLC0415
        pg_data = pg_cache_get("pp_pitches", cache_key)
        if pg_data and isinstance(pg_data, list):
            logger.debug(
                "[PredictPlus] Postgres cache hit — mlbam=%d season=%d (%d pitches)",
                mlbam_id, season, len(pg_data),
            )
            # Backfill Redis so next call is faster
            try:
                import redis as _redis_lib  # noqa: PLC0415
                import json as _json  # noqa: PLC0415
                _r_url = os.environ.get("REDIS_URL") or os.environ.get("REDIS_PRIVATE_URL", "")
                if _r_url:
                    _redis_lib.from_url(_r_url, decode_responses=True).set(
                        cache_key, _json.dumps(pg_data), ex=604800
                    )
            except Exception:
                pass
            return pg_data
    except Exception:
        pass  # layer_cache_helper unavailable — fall through

    # ── L3: Live Savant fetch ────────────────────────────────────────────────
    params = {
        "all": "true",
        "player_type": "pitcher",
    ...
    try:
        ...
        if pitches:
            ...
            try:
                ...
                if _r_url:
                    _redis_lib.from_url(_r_url, decode_responses=True).set(
                        cache_key, _json.dumps(pitches), ex=604800  # 7 days
                    )
            except Exception:
                pass
            # ── Write-through to Postgres (L2) ────────────────────────────
            try:
                from layer_cache_helper import pg_cache_set  # noqa: PLC0415
                pg_cache_set("pp_pitches", cache_key, pitches)
            except Exception:
                pass
