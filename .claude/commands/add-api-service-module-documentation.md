---
name: add-api-service-module-documentation
description: Workflow command scaffold for add-api-service-module-documentation in mework.
allowed_tools: ["Bash", "Read", "Write", "Grep", "Glob"]
---

# /add-api-service-module-documentation

Use this workflow when working on **add-api-service-module-documentation** in `mework`.

## Goal

Adds documentation for a new API service module.

## Common Files

- `.claude/commands/add-api-service-module-documentation.md`

## Suggested Sequence

1. Understand the current state and failure mode before editing.
2. Make the smallest coherent change that satisfies the workflow goal.
3. Run the most relevant verification for touched files.
4. Summarize what changed and what still needs review.

## Typical Commit Signals

- Create or update a markdown file in .claude/commands/ with the API service module documentation.

## Notes

- Treat this as a scaffold, not a hard-coded script.
- Update the command if the workflow evolves materially.