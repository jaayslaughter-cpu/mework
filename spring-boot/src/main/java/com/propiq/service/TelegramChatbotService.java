...
public class TelegramChatbotService {
    ...
    // ─────────────────────────────────────────────────────────
    //  LONG POLL — fires every 3 seconds
    // ─────────────────────────────────────────────────────────
    ...
    private void handlePlayerLookup(String query, String chatId) {
        ...
        // Case-insensitive fuzzy match on prop key (format: PlayerName_PropType)
        List<Map.Entry<String, AnalyzerCacheItem>> matches = cache.entrySet().stream()
            ...
            .sorted((a, b) -> Double.compare(b.getValue().getEdgePct(), a.getValue().getEdgePct())) // best EV first
            .limit(3)
            .toList();
        ...
        // Return top match as full report; additional matches as brief summaries
        alertService.sendTelegramMessage(chatId, formatIntelligenceReport(matches.get(0).getKey(), matches.get(0).getValue()));
    ...
    private void handleTopPlays(String chatId) {
        ...
        cache.entrySet().stream()
            .filter(e -> e.getValue().getEdgePct() >= 3.0)   // min 3% EV threshold
    ...
    private String formatIntelligenceReport(String propKey, AnalyzerCacheItem item) {
        ...
        String recommendation = buildRecommendation(item);
    ...
    private int confidenceScore(double modelProb) {
        // Maps 50–100% model probability → 1–10 scale
    ...
    private String formatMatchupContext(String raw) {
        ...
        // Prefix each key metric with an emoji bullet
