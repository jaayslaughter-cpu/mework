...
def build_parlay(
    ...
    # Overall parlay EV (geometric average of per-leg EVs)
    ev_pct    = sum(l.ev_pct for l in selected) / len(selected)
    joint_prob = math.prod(l.implied_prob for l in selected)
    conf       = min(10, max(1, round(joint_prob * 15)))  # scale to 1-10
    ...
    def run(self, date_str: str | None = None) -> None:
        ...
        for agent in AGENT_CONFIGS:
            ...
            if ev < MIN_EV_PCT:
                ...
                continue
            n = len(parlay["legs"])
            logger.info("[%s] %d-leg parlay EV=%.1f%% → SEND", agent["name"], n, ev)
            if not self.dry_run:
